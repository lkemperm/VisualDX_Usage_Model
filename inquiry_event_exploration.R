...
require(xts)
inquiry.ts <- xts(as.numeric(inquiry_table$duration), order.by=inquiry_table$startTime, start = min(inquiry_table$startTime),
                  end = max(inquiry_table$startTime) ,frequency = 12)
plot(inquiry.ts)
